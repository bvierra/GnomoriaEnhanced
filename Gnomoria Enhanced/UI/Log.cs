    ...
    public sealed class ListBoxLog : IDisposable
    {
        ...
        private void DrawItemHandler(object sender, DrawItemEventArgs e)
        {
            if (e.Index >= 0)
            {
                ...
                if (logEvent.LogLevel == LogLevel.Critical)
                {
        ...
        private string FormatALogEventMessage(LogEvent logEvent, string messageFormat)
        {
            ...
            return string.Format(messageFormat,
                ...
                /* {5} */ LevelName(logEvent.LogLevel)[0],
                /* {6} */ LevelName(logEvent.LogLevel),
                /* {7} */ (int)logEvent.LogLevel,
        ...
        private void CopyToClipboard()
        {
            if (_listBox.SelectedItems.Count > 0)
            {
                ...
                {
                    selectedItemsAsRTFText.AppendFormat(@"{{\f0\fs16\chshdng0\chcbpat{0}\cb{0}\cf{1} ", (logEvent.LogLevel == LogLevel.Critical) ? 2 : 1, (logEvent.LogLevel == LogLevel.Critical) ? 1 : ((int)logEvent.LogLevel > 5) ? 6 : ((int)logEvent.LogLevel) + 1);
